...
function getPdfPath() {
    ...
    $pdfPath = "";
    if (isset($_FILES['pdf'])) {
        $pdf_name = explode('.', $_FILES['pdf']['name']);
        $pdf_count_array = count($pdf_name);
        $pdf_count_array -= 1;

        if ($pdf_name[$pdf_count_array] == "pdf" && $_FILES['pdf']['size'] <= 5242880) {
            $pdfPathUpload = "../../resources/files/{$fileName}.pdf";
            move_uploaded_file($_FILES['pdf']['tmp_name'], $pdfPathUpload);
            $pdfPath = "resources/files/{$fileName}.pdf";
        }
    }
    return $pdfPath;
}
...
function getUpdatedPdfPath() {
    ...
    if (isset($_FILES['pdf'])) {
        ...
        if ($pdf_name[$pdf_count_array] == "pdf" && $_FILES['pdf']['size'] <= 5242880) {
            ...
            $pdfPath = filter_input(INPUT_POST, 'pdf_path', FILTER_SANITIZE_STRING);
        }
    }
    return $pdfPath;
}
