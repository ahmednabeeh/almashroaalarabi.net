            ...
            <div id="midBar">
                <div id="subPages">
                    ...
                    <div style="text-align: center; margin-top: 10px;">
                        <?php
                        if (mysqli_num_rows(mysqli_query($serverConnection, "SELECT news_id, title, content, image, views FROM news WHERE category = 'نشاطات المكتب' AND display = 1 AND display_phone = 0 ORDER BY news_id DESC LIMIT  $start, " . PERPAGE)) != 0) {
                            $defaultActivities = mysqli_query($serverConnection, "SELECT news_id, title, content, image, views FROM news WHERE category = 'نشاطات المكتب' AND display = 1 AND display_phone = 0 ORDER BY news_id DESC LIMIT  $start, " . PERPAGE);
                            $counter = 1;
                            while ($defaultActivitesNewsResult = mysqli_fetch_array($defaultActivities)) {
                                echo "<div style=\"display: inline-block; width: 380px; margin-left: 10px; margin-bottom: 10px; background-color: #fff; text-align: left;\">";
                    ...
                    <div class="pagination" style="background-color: #F00;">
                        ...
                        $nextPage = $page + 1;
                        $pagesNum = ceil(mysqli_num_rows(mysqli_query($serverConnection, "SELECT `news_id` FROM `news` WHERE `category` = 'بيانات ومواقف'")) / PERPAGE);
