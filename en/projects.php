            ...
            <div id="midBar">
                <div id="subPages">
                    ...
                    <div style="text-align: center; margin-top: 10px;">
                        <?php
                        if (mysqli_num_rows(mysqli_query($serverConnection, "SELECT news_id, title, content, image, views FROM news WHERE category = 'المشاريع' AND display = 1 AND display_phone = 0 ORDER BY news_id DESC LIMIT  $start, " . PERPAGE)) != 0) {
                            $defaultActivities = mysqli_query($serverConnection, "SELECT news_id, title, content, image, views FROM news WHERE category = 'المشاريع' AND display = 1 AND display_phone = 0 ORDER BY news_id DESC LIMIT  $start, " . PERPAGE);
                            $counter = 1;
                    ...
                    <div class="pagination" style="background-color: #F00;">
                        ...
                        $nextPage = $page + 1;
                        $pagesNum = ceil(mysqli_num_rows(mysqli_query($serverConnection, "SELECT `news_id` FROM `news` WHERE `category` = 'المشاريع'")) / PERPAGE);
