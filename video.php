...
<html>
    ...
    <body>
        ...
        <div id="wrapper">
            ...
            <div id="midBar">
                <div id="subPages">
                    <div class="title">مكتبة الفيديو</div>
                    <div style="text-align: center; margin-top: 10px;">
                        <?php
                        $video = "SELECT `video_id`, `title`, `image` FROM `video` WHERE `display` = 1 AND `display_phone` = 0 ORDER BY `video_id` DESC LIMIT 15";
                        $video = mysqli_query($serverConnection, $video);
                        $counter = 1;
                        if (mysqli_num_rows(mysqli_query($serverConnection, "SELECT `video_id`, `title`, `image` FROM `video` WHERE `display` = 1 AND `display_phone` = 0 ORDER BY `video_id` DESC LIMIT 15")) != 0) {
                            while ($videoResult = mysqli_fetch_array($video)) {
                                $imagePath = $videoResult['image'];
                                $titleExplodeArray = explode(" ", $videoResult['title']);
                                $title = "";
                                for ($i = 0; $i <= 5; $i++) {
						...
						<div><a href=\"video_player.php?video_id=$videoResult[video_id]\" target=\"_blank\">
						<img style=\"border: 1px solid #5c7c96; padding: 5px;\" src=\"$imagePath\" width=\"250\" height=\"155\" /></a></div><div style=\"background-color: #ddd; color: #2e3e4b; padding: 5px; text-align: center; width: 253px;\">$title</div>
						</div>";
                            }
                        } else {
                            echo "<h2 style=\"text-align: center; color: #f00\">لايوجد فيديوهات</h2>";
                        }
                        ?>
                    </div>
                </div>
