                    ...
                    <div style="text-align: center; margin-top: 10px;">
                        ...
                        if (mysqli_num_rows(mysqli_query($serverConnection, "SELECT `gallery_id`, `title` FROM `gallery` WHERE `display` = 1 AND `display_phone` = 0 ORDER BY `gallery_id` DESC LIMIT 30")) != 0) {
                            while ($fetchGalleryResult = mysqli_fetch_array($fetchGallery)) {
                                ...
                                echo "<div style=\"display: inline-block; margin: 15px; vertical-align: top; width: 260px;\"><a href=\"gallery_viewer.php?gallery_id=$fetchGalleryResult[gallery_id]\" target=\"_blank\"><img style=\"border: 1px solid #5c7c96; padding: 5px;\" src=\"$imagePath\" width=\"250\" height=\"150\" /></a><div style=\"color: #2e3e4b; padding: 5px; text-align: center; width: 250px;\">$title</div></div>";
                            }
                        } else {
                            echo "<h2 style=\"text-align: center;\">لايوجد البومات صور</h2>";
                        }
                        ?>
                    </div>
                </div>
